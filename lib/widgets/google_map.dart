...
class _GoogleMapScreenState extends State<GoogleMapScreen> {
  ...
  void _handleTap(LatLng tappedPoint) async {
    ...
    // Get the place id from coordinates
    final placeId = await getPlaceIdFromLatLng(
    ...
    if (placeId == null) {
      ...
      if (mounted) {
        setState(() {
          ...
          _placeDetailsError = 'Could not find location details';
        });
      }
      return;
    }
    ...
    if (details == null) {
      setState(() {
        _isLoadingPlaceDetails = false;
        _placeDetailsError = 'Could not load location details';
      });
      return;
    }
  ...
  Widget _loadingPlaceDetailsWidget() {
    return Align(
  ...
  Widget _errorPlaceDetailsWidget() {
    return Align(
      alignment: Alignment.bottomCenter,
      child: SafeArea(
        top: false,
          ...
          child: _blurBgWrapper(
            ...
            child: Row(
              ...
              children: [
                Expanded(
                  child: Text(
                    _placeDetailsError ?? 'Error loading location',
                    style: TextStyle(
                      ...
                      children: [
                        ..._selectedPlaceDetails!['photos'].asMap().entries.map((
                          ...
                          return _photoWidget(
                            photo: photo,
                            onTap: () {
                              // TODO: Implement image viewer if needed
                              // For now, just show a snackbar with the photo index
    ...
    Widget buildWidget = SizedBox(
      ...
      child: Stack(
        children: [
          ...
          _searchWidget(),
          // Show loading indicator when fetching place details
          if (_isLoadingPlaceDetails && _selectedPosition != null)
            _loadingPlaceDetailsWidget(),
          // Show error if place details failed to load
          if (_placeDetailsError != null && _selectedPosition != null)
            _errorPlaceDetailsWidget(),
          // Show place details when loaded
